...
thread_pool_new(int max_thread_count, struct thread_pool **pool)
{
  ...
  pthread_t* threads = malloc(max_thread_count * sizeof(pthread_t));
  ...
  struct task_queue* tq = malloc(sizeof(struct task_queue));
  check_malloc(tq);
...
void* worker(void* arg) {
  ...
  while (true) {
    pthread_mutex_lock(&pool->lock);
    while (pool->task_queue->size == 0 && !pool->finish) {
...
resize_queue_if_needed(struct task_queue* tq) {
  if (tq->size + 1 > tq->capacity) {
    ...
    struct thread_task** data = realloc(tq->data, new_size);
    check_malloc(data);
    tq->data = data; 
  }
  return tq;
...
thread_pool_push_task(struct thread_pool *pool, struct thread_task *task)
{
  ...
  if (pool->count == pool->active && pool->count < pool->max_thread_count) {
    pthread_create(&pool->threads[pool->count++], NULL, worker, pool);
  }
...
{
  struct thread_task* ts = malloc(sizeof(struct thread_task));
  check_malloc(ts);
